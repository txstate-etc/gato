  ...
  [#else]
    [#list components as slide]
      ...
      [#else]
        [#assign tabtitle = "Slide " + (slide_index + 1)]
      [/#if]
      <button class="visuallyhidden" role="tab" id="slidetab${gf.uuidToHtmlId(slide.@id)}" tabindex="0" aria-controls="slidepanel${gf.uuidToHtmlId(slide.@id)}" aria-selected="${(slide_index == 0)?then("true", "false")}">
        <span class="visuallyhidden">${tabtitle}</span>
      </button>
    [/#list]
  [/#if]
