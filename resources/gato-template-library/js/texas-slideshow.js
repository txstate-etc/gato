function texas_slideshow(box, opts) {
...
texas_slideshow.prototype.activate = function(index) {
...
texas_slideshow.prototype.next = function() {
	var next = (this.current == this.count - 1 ? 0 : this.current + 1);
	this.activate(next);
};

texas_slideshow.prototype.prev = function() {
	var prev = (this.current == 0 ? this.count - 1 : this.current - 1);
	this.activate(prev);
};

texas_slideshow.prototype.tooltip = function(lnk, tip) {
...
texas_slideshow.prototype.hidetooltip = function () {
	if (this.tipdiv) this.tipdiv.setStyle({display: 'none'});
};

texas_slideshow.prototype.initeditbars = function () {
	...
	container.select('.slide-container .mgnlEditor.mgnlEditorBar.component').each(function(itm) {
		cnt += 1;
		top += itm.getHeight();
