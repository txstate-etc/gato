  ...
  <ul class="simplemenu">
    [@navloop cmsfn.children(homepage, 'mgnl:page') ; page]
      <li class="${gf.hasNavChildren(page)?string('haschildren', '')}">
        <a class="${navfn.isActive(content, page)?string('active', '')}" href="${cmsfn.link(page)}">${gf.nodeTitle(page)}</a>
        [#if gf.hasNavChildren(page)]
...
[#macro breadcrumbs hidetxstate=false isCalico=false]
  <div class="breadcrumbs">
    ...
    [#if isCalico]
      [#assign parentOrgArea = homepage["organization-info"]!homepage.parentOrganization!]
    [#else]
      [#assign parentOrgArea = homepage.parentOrganization!homepage["organization-info"]!]
    [/#if]
    [#if parentOrgArea?has_content && cmsfn.children(parentOrgArea)?size gt 0 ]
