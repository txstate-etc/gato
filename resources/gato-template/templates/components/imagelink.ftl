<a href="${gf.filterUrl(content.link)}" class="gato-imagelink">
  ...
  [/#if]
  <img src="${gf.getImgDefault(rolloverImage)}" sizes="${ctx.maxwidth!'100vw'}" srcset="${gf.getSrcSet(rolloverImage)}" alt="${content.imageAlt}" title="${content.imageAlt}" class="rollover" />
</a>
