...
[#else]
  ...
  <input name="${title}" id="${title}-dummy-item" type="${content.type}" style="display:none" value="" checked="checked" />
  [#assign i = 0]
  [#list content.options as option]
    <div class="txst-form-selection-item">
      ...
      [#assign label = option?keep_after("|")]
      [#if !label?has_content][#assign label = value][/#if]
      <input name="${title}" id="${title}${i}" type="${content.type}" class="${content.type}" value="${value}" />
