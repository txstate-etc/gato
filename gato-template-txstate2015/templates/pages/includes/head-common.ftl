<link rel="stylesheet" href="${ctx.contextPath}/.resources/gato-template-txstate2015/resources/css/normalize.min.css" type="text/css">
<link rel="stylesheet" href="${ctx.contextPath}/.resources/gato-template-txstate2015/resources/css/boilerplate.min.css" type="text/css">
<link rel="stylesheet" type="text/css" href="//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.1.0/css/font-awesome.min.css">
<link rel="stylesheet" href="${ctx.contextPath}/.resources/gato-template-txstate2015/resources/css/gato.css" type="text/css">

<script type="text/javascript" src="//code.jquery.com/jquery-latest.min.js"></script>

<script type="text/javascript" src="//bigspotteddog.github.io/ScrollToFixed/jquery-scrolltofixed-min.js"></script>
<script type="text/javascript" src="//cdnjs.cloudflare.com/ajax/libs/respond.js/1.4.2/respond.min.js"></script>
<script type="text/javascript" src="//cdnjs.cloudflare.com/ajax/libs/fastclick/1.0.2/fastclick.min.js"></script>
<script type="text/javascript" src="${ctx.contextPath}/.resources/gato-template-txstate2015/resources/js/slideout.js"></script>
<script type="text/javascript" src="${ctx.contextPath}/.resources/gato-template-txstate2015/resources/js/common.js"></script>
